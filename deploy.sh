#!/usr/bin/env bash
# ── Gleap Dashboard — Cloudflare Pages deploy script ─────────────────────────
#
# Usage (local or CI):
